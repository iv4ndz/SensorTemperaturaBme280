...
#Escritura en Base de datos MSSQL

def write_mssql(temperature,humidity,pressure):
	db = pyodbc.connect('DRIVER={FreeTDS};SERVER='+server+';DATABASE='+database+';UID='+username+';PWD='+ password+';TDS_Version=8.0;')

	# Crea un cursor usando la conexion
	cursor = db.cursor()

	# Genera la consulta SQL para  insertar el registro
	sql = "INSERT INTO datos (fecha, temperatura, humedad,presion) values (getdate(),'" + str(temperature) + "','" + str(humidity) + "','" + str(pressure) + "');"
...
try:
	...
	while True:
		...
		if humidity is not None and temperature is not None:
			write_log("BME280 Sensor - Temperatura: %s" % str(temperature))
			write_log("BME280 Sensor - Humedad:  %s" % str(humidity))
			write_log("BME280 Sensor - Presion:  %s" % str(pressure))
			write_mssql(temperature,humidity,pressure)
		else:
